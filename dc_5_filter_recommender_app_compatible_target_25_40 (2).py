...
def main():
    ...
    if TODAY_POOL_CSV:
        ...
        md_lines = []
        md_lines.append(f"# DC5 Recommender — Today
")
        md_lines.append(f"**Seed:** `{base_env['seed']}`  |  **Sum:** {base_env['seed_sum']} ({base_env['seed_sum_category']})  |  **Structure:** {classify_structure(seed_list)}  |  **Spread:** {base_env['spread_seed']}  |  **Parity:** {parity_major}
")
        md_lines.append(f"**Hot:** {sorted(base_env['hot_digits'])}  |  **Cold:** {sorted(base_env['cold_digits'])}  |  **Due:** {sorted(base_env['due_digits'])}
")
        md_lines.append("")
        md_lines.append(f"**Applicable filters now:** {len(applicable)}  |  **Target:** < {TARGET_MAX+1}  |  **Winner preserved:** {'✅ YES' if winner_kept else '❌ NO'}
")
        md_lines.append("")
