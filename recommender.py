    ...
    if TODAY_POOL_CSV:
        ...
        if applied_steps:
            for r in applied_steps:
                md_lines.append("- Step {}: **{}** — {}  · eliminated **{}** → remaining **{}**".format(r['step'], r['filter_id'], r['name'], r['eliminated_now'], r['remaining']))
        else:
        ...
        if not avoid_df.empty:
            for _, row in avoid_df.head(12).iterrows():
                md_lines.append("- **{} + {}**  · pair_risk={}  (both_blocked/CoApp={}/{})".format(row['filter_id_1'], row['filter_id_2'], row['pair_risk'], row['both_blocked_n'], row['co_applicable_n']))
        else:
