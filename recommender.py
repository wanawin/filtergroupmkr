...
def historical_risk_for_applicable(filters: List[FilterDef], winners: List[str], idx_now: int):
    env_now = build_env_for_draw(idx_now, winners)
...
def main(
        ...
        if pr > 0:
            ...
            if co_app>0:
                avoid_rows.append({
                    ...
                    "pair_risk": round(both_block / max(co_app,1), 6)
                })
    ...
    winner_today = winners[idx_now] if pool is not None else None
    ...
    if today_pool_csv:
        ...
        # ======== One-pager (HTML) ========
        # Keep CSS as a plain string (no formatting braces problems)
...
.badge.bad { background:#fef2f2; }
        ...
        applied_html = df_to_html_table(applied_df, ["step","filter_id","name","eliminated_now","remaining"],
                                        "No safe reduction steps.", 50)
        skipped_html = df_to_html_table(skipped_df, ["filter_id","name","skipped_reason"],
                                        "No filters were skipped.", 50)
        avoid_html   = df_to_html_table(avoid_sorted, ["filter_id_1","filter_id_2","pair_risk","both_blocked_n","co_applicable_n"],
