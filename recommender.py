...
def main(
    ...
    if pool is not None:
        for f in applicable.values():
            # Dry-run; do not commit to pool yet
            _, elim = apply_filter_to_pool(f, env_now, pool)
            est_elims[f.fid] = elim
    else:
        est_elims = {fid: 0 for fid in applicable}

    # ---- Rank applicable: support first, then lower risk, then higher estimated eliminations, then ID
    ranked = sorted(
        applicable.values(),
        key=lambda f: (
            -support.get(f.fid, 0),
    ...
    if not do_not_df.empty:
        do_not_df = do_not_df.sort_values(
            ...
            key=lambda s: s.map(tier_order) if s.name == "tier" else s  # ✅ no NoneType
        )
