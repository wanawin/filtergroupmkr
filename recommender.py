...
TODAY_POOL_CSV: Optional[str] = "today_pool.csv"  # default local pool
TARGET_MAX = 44
...
# Synthetic pool for tie-breaks when no pool provided
def _sample_pool_for_estimates(env_now: Dict[str, object], n: int = 5000) -> List[str]:
    rng = random.Random(int(''.join(map(str, env_now['seed_digits_list']))))
    seen = set()
    out = []
    while len(out) < n:
...
def main(
    ...
    # Optional override for today's seed and previous draws
    if override_seed:
    ...
    # Support counts over same indices
    ...
    if pool is None:
        pool_for_est = _sample_pool_for_estimates(env_now, n=5000)
    else:
        pool_for_est = pool

    est_elims = {}
    for f in applicable.values():
        _, elim = apply_filter_to_pool(f, env_now, pool_for_est)
        est_elims[f.fid] = elim

    # Rank: support first, then lower risk, then higher estimated eliminations, then ID
    ranked = sorted(
        applicable.values(),
        key=lambda f: (
            -support.get(f.fid, 0),
    ...
    # Reduction
    seq_rows = []
    base_env = build_env_for_draw(idx_now, winners)
    winner_today = None
    if force_keep_combo:
    ...
    if today_pool_csv and Path(str(today_pool_csv)).exists():
        pd.DataFrame([r for r in seq_rows if r["eliminated_now"] not in (None,0)]).to_csv(
            OUTPUT_DIR / "pool_reduction_log.csv", index=False
        )

    # One-pager outputs kept out for brevity in canvas edits (HTML/MD can be re-added)
